...
// Auto-assign closest person + nearest-neighbor multi-stop routes (TSP-style)
...
function renderMarkers() {
  ...
  state.people.forEach((p) => {
    ...
    const marker = L.marker([p.lat, p.lng], { icon }).addTo(state.map).bindPopup(
...
function drawRoutes(store, people) {
  ...
  people.forEach((p, idx) => {
    ...
    const line = L.polyline([[p.lat, p.lng], [store.lat, store.lng]], {
...
function renderStoreList(filter = "") {
  ...
  const filtered = state.stores.filter((s) =>
    s.fullName.toLowerCase().includes(q) || s.city.toLowerCase().includes(q) ||
    s.chain.toLowerCase().includes(q) || s.projectCode.toLowerCase().includes(q) ||
    s.brand.toLowerCase().includes(q) || (s.market && s.market.toLowerCase().includes(q))
  );
  ...
  list.innerHTML = shown.map((s) => {
    ...
    return `<div class="list-item ${s.id === state.selectedStoreId ? "active" : ""}" data-id="${s.id}">
      ...
      <div class="meta">${s.projectCode} · ${s.typicalHours}h · ${s.city}</div>
...
function selectStore(storeId) {
  ...
  $("selectedStoreInfo").innerHTML = `<div class="name">${store.fullName}</div>
    <div class="meta">${store.projectCode}<br>${store.address}, ${store.city}<br>
    Brand: <strong>${store.brand}</strong> · ${store.windowStart} → ${store.windowEnd}</div>`;
...
function renderResults(store) {
  ...
  $("assignmentSummary").innerHTML = `
    <div><strong>Auto-selected</strong> closest ${primaries.length} (scope + drive)</div>
  ...
  }).join("") || '<div class="hint">No available people within max drive. Increase max drive.</div>';
...
function confirmAssignment() {
  ...
  state.assignments.push({
    ...
    status: "Scheduled", lat: store.lat, lng: store.lng, timestamp: new Date()
  });
  state.selectedStoreId = null; state.candidates = [];
  $("assignmentPanel").style.display = "none"; $("resultsPanel").style.display = "none";
  clearRoutes(); updateStats(); renderStoreList($("storeSearch").value); renderAssignedList(); renderMarkers(); refreshMerchIfNeeded();
}

function autoAssignAllVisible() {
  let assigned = 0;
  const unassigned = state.stores.filter((s) => !state.assignments.some((a) => a.storeId === s.id)).slice(0, 25);
  unassigned.forEach((store) => {
    const ranked = rankCandidates(store);
    if (!ranked.length) return;
    const pick = ranked[0];
    const person = state.people.find((x) => x.id === pick.id);
  ...
  updateStats(); renderStoreList($("storeSearch").value); renderAssignedList(); renderMarkers(); refreshMerchIfNeeded();
...
function renderAssignedList() {
  ...
  if (!list) return;
  if (!state.assignments.length) { list.innerHTML = '<div class="hint">Select a store — closest person auto-picked</div>'; return; }
  list.innerHTML = state.assignments.slice().reverse().slice(0, 30).map((a) =>
    `<div class="list-item"><div class="name">${a.storeName}</div>
     ...
     <span class="tag assigned">Scheduled</span></div>`).join("");
}
...
function updateStats() {
  ...
  if (state.assignments.length) {
    $("statDrive").textContent = Math.round(
      state.assignments.reduce((s, a) => s + a.totalDriveMin / Math.max(1, a.personIds.length), 0) / state.assignments.length
    );
  } else $("statDrive").textContent = "—";
}

function resetAll() {
  state.people.forEach((p) => { const o = window.DEMO_DATA.PEOPLE.find((x) => x.id === p.id); p.status = o ? o.status : "available"; });
  state.assignments = []; state.selectedStoreId = null; state.candidates = [];
  $("assignmentPanel").style.display = "none"; $("resultsPanel").style.display = "none";
  clearRoutes(); updateStats(); renderStoreList(); renderAssignedList(); renderMarkers(); refreshMerchIfNeeded();
}
...
function switchView(view) {
  ...
  if (view === "merch") {
    initMerchMap(); populatePersonPicker(); renderWeekBar(); renderMerchAssignments();
...
function populatePersonPicker() {
  ...
  sel.innerHTML = '<option value="">— Choose merchandiser —</option>' +
    state.people.slice().sort((a, b) => a.name.localeCompare(b.name))
      .map((p) => `<option value="${p.id}">${p.name} (${p.market})</option>`).join("");
...
function selectPerson(personId) {
  ...
  if (p) {
    ...
    $("merchName").textContent = "Select a person"; $("merchMeta").textContent = "Choose a merchandiser"; $("merchAvatar").textContent = "?";
  }
...
function renderWeekBar() {
  ...
  container.innerHTML = days.map((d, i) => {
    const count = state.selectedPersonId
      ? state.assignments.filter((a) => a.personIds.includes(state.selectedPersonId) && a.dayIndex === i).length : 0;
    return `<button type="button" class="day-chip ${i === state.selectedDayIndex ? "active" : ""}" data-day="${i}">
      ${labels[i]}(${count})<span class="count">${d.getMonth() + 1}/${d.getDate()}</span></button>`;
  }).join("");
  container.querySelectorAll(".day-chip").forEach((el) => el.addEventListener("click", () => {
    state.selectedDayIndex = parseInt(el.dataset.day, 10); renderWeekBar(); renderMerchAssignments();
  }));
}
...
function renderMerchAssignments() {
  ...
  if (!state.selectedPersonId) {
    ...
    $("dayCount").textContent = "0"; clearMerchMarkers(); return;
  }
  ...
  if (!items.length && !state.assignments.length) {
    ...
    const nearby = state.stores.map((s) => ({ s, d: haversineMiles(person.lat, person.lng, s.lat, s.lng) }))
      .filter((x) => x.d < 80).sort((a, b) => a.d - b.d).slice(0, 3);
    items = nearby.map((n, idx) => ({
      storeId: n.s.id, storeName: n.s.fullName, projectCode: n.s.projectCode, brand: n.s.brand,
      projectHours: n.s.typicalHours, startTime: ["09:00", "11:00", "13:30"][idx] || "15:00",
      endTime: ["10:30", "12:00", "15:00"][idx] || "16:30", status: "Scheduled",
      lat: n.s.lat, lng: n.s.lng, demo: true
    }));
  }
  const person = state.people.find((p) => p.id === state.selectedPersonId);
  const stops = items.map((a) => {
    const store = state.stores.find((s) => s.id === a.storeId);
    return { ...a, lat: a.lat || (store && store.lat), lng: a.lng || (store && store.lng) };
  }).filter((s) => s.lat != null);
  ...
  const routeSummary = route.ordered.length > 1
    ? `<div class="summary" style="margin-bottom:10px"><strong>Optimized route</strong> (nearest-neighbor)<br>${route.ordered.length} stops · ~${route.totalMiles} mi · ~${route.totalMin} min drive</div>` : "";
...
function updateMerchMap(items, person, route) {
  if (!state.merchMap || !person) return;
  clearMerchMarkers();
  const home = L.marker([person.lat, person.lng], { icon: createIcon("person", "H") }).addTo(state.merchMap).bindPopup(`<strong>${person.name}</strong><br>Home`);
  state.merchMarkers.push(home);
  const bounds = [[person.lat, person.lng]];
  let prev = { lat: person.lat, lng: person.lng };
  items.forEach((a, i) => {
    if (a.lat == null) return;
    const m = L.marker([a.lat, a.lng], { icon: createIcon("store", String(i + 1)) }).addTo(state.merchMap).bindPopup(`<strong>${i + 1}. ${a.storeName}</strong>`);
    state.merchMarkers.push(m);
    bounds.push([a.lat, a.lng]);
    const line = L.polyline([[prev.lat, prev.lng], [a.lat, a.lng]], { color: "#2563eb", weight: 3, opacity: 0.75 }).addTo(state.merchMap);
...
function refreshMerchIfNeeded() {
  if (state.currentView === "merch") { renderWeekBar(); renderMerchAssignments(); }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (sidebar && !$("autoAllBtn")) {
    ...
    wrap.innerHTML = '<h2>Bulk Auto-Assign</h2><p class="hint">Closest available person (scope + drive) for up to 25 open stores.</p>';
    const btn = document.createElement("button");
    btn.id = "autoAllBtn"; btn.className = "btn secondary full"; btn.textContent = "Auto-assign closest to many stores";
    btn.addEventListener("click", autoAssignAllVisible);
  ...
  window.addEventListener("orientationchange", () => {
    setTimeout(() => { if (state.map) state.map.invalidateSize(); if (state.merchMap) state.merchMap.invalidateSize(); }, 200);
  });
